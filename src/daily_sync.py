...
Syncs the default/unscoped tenant plus every Rapid7-managed organization
that rapid7-mcp-server actually tracks as a tenant (see discover_tenants's
vm_sync_allowlist filtering -- falls back to every managed org, discovered
live, if that allowlist file isn't present) across all 4 export types
(vulnerability, policy, remediation, asset_software), then writes a
...
# Written by rapid7-mcp-server's scripts/export-vm-sync-allowlist.ts, regenerated by
# run-daily-sync.sh right before this module runs -- lists exactly the Rapid7 org ids that
# project tracks as tenants (its tenants.vm_org_id column), so this script doesn't sync every
# managed org under the account, only the ones actually used. See discover_tenants().
VM_SYNC_ALLOWLIST_PATH = Path(
    os.environ.get("VM_SYNC_ALLOWLIST_PATH", str(_DATA_DIR / "vm_sync_allowlist.json"))
)
...
def _load_vm_sync_allowlist() -> Optional[set]:
    """Set of Rapid7 org ids rapid7-mcp-server actually tracks as tenants, or None if the
    allowlist file is missing/unreadable -- callers must treat None as "sync everything"
    (today's behavior), not as an empty allowlist, so a stale/not-yet-generated file can't
    silently stop every org from syncing."""
    try:
        raw = json.loads(VM_SYNC_ALLOWLIST_PATH.read_text())
    except FileNotFoundError:
        logger.warning(
            "vm sync allowlist not found at %s -- syncing every managed org (unfiltered)",
            VM_SYNC_ALLOWLIST_PATH,
        )
        return None
    except (json.JSONDecodeError, OSError) as e:
        logger.warning(
            "vm sync allowlist at %s unreadable (%s) -- syncing every managed org (unfiltered)",
            VM_SYNC_ALLOWLIST_PATH,
            e,
        )
        return None
    return {t["vm_org_id"] for t in raw.get("tenants", []) if t.get("vm_org_id")}


def discover_tenants() -> List[Dict[str, str]]:
    """Default/unscoped tenant plus every live-discovered managed organization in this
    account's own configured region that rapid7-mcp-server actually tracks as a tenant
    (see _load_vm_sync_allowlist). Also filtered the same way get_organization_ids's MCP
    tool filters by default -- confirmed live that an unfiltered list includes cross-region
    ...
    allowlist = _load_vm_sync_allowlist()
    if allowlist is not None:
        skipped = [org.get("name", org["id"]) for org in orgs if org["id"] not in allowlist]
        if skipped:
            logger.info("vm sync allowlist: skipping %d org(s) not tracked as tenants: %s", len(skipped), skipped)
        orgs = [org for org in orgs if org["id"] in allowlist]
